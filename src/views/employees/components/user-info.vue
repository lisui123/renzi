      ...
      <el-row class="inline-info">
        <el-col :span="12">
          <el-form-item label="员工头像">
            <!-- 放置上传图片 -->
            <ImageUpload ref="refUpload"></ImageUpload>
          </el-form-item>
        </el-col>
      </el-row>
      <!-- 保存个人信息 -->
    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item label="员工照片">
          <!-- 放置上传图片 -->
          <ImageUpload ref="perRef"></ImageUpload>
        </el-form-item>
...
export default {
  ...
  methods: {
    // 保存上半部分用户信息
    async saveUser() {
      const fileList = this.$refs.refUpload.fileList;
      // 判断图片是否上传成功》》upload属性是否存在
      // 是否包含未上传完毕的元素
      if (fileList.some((item) => !item.upload)) {
        // 错误提示
        this.$message.warning("图片还没有上传成功");
        return;
      }
      // 全部完成
    ...
    async getPersonalDetail() {
      ...
      //2.点击跟新的时候，需要获取组件内部的fileList进行提交
      this.$refs.perRef.fileList = [
        {
